...
import { LocalRequest, LocalRequestSource, LocalResponse, LocalResponseSource, OutgoingRequestsController } from "@vermascht/consumption";
import { RequestItemGroupJSON, RequestJSON, ResponseItemGroupJSON, ResponseJSON } from "@vermascht/content";
...
export class GetOutgoingRequestsUseCase extends UseCase<GetOutgoingRequestsRequest, LocalRequestDTO[]> {
    private static readonly queryTranslator = new QueryTranslator({
        ...
        alias: {
            ...
            // response.source.type
            [`${nameof<LocalRequestDTO>((x) => x.response)}.${nameof<LocalResponseDTO>((x) => x.source)}.${nameof<LocalResponseSourceDTO>((x) => x.type)}`]:
