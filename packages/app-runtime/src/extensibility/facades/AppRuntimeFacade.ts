...
export abstract class AppRuntimeFacade {
    ...
    protected async parseErrorResult<T>(result: Result<any>): Promise<Result<T, UserfriendlyApplicationError>> {
        ...
        if (userfriendlyMessageResult.isSuccess) {
            return UserfriendlyResult.fail<T, UserfriendlyApplicationError>(
                UserfriendlyApplicationError.fromError(result.error, userfriendlyMessageResult.value)
            )
        }
        return UserfriendlyResult.fail<T, UserfriendlyApplicationError>(
            UserfriendlyApplicationError.fromError(result.error, result.error.code)
        )
    }
