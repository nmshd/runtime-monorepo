...
export class RelationshipEventingRejectTest extends AbstractTest {
    public run(): void {
        const that = this
        ...
        describe("RelationshipEventingRejectTest", function () {
            ...
            it("should fire events on the templator when relationship request was rejected (by templator itself)", async function () {
                ...
                expect(relationshipChangedEvent.data).to.exist
                expect(relationshipChangedEvent.data.id).to.eq(relationshipId)
                expect(relationshipChangedEvent.data.status).to.eq(RelationshipStatus.Rejected)
            })

            it("should fire events on the requestor when relationship request was rejected", async function () {
                ...
                expect(onboardingChangeReceivedEvent).instanceOf(OnboardingChangeReceivedEvent)
                expect(onboardingChangeReceivedEvent.data).to.exist
                expect(onboardingChangeReceivedEvent.data.change.status).to.eq(RelationshipChangeStatus.Rejected)
                expect(onboardingChangeReceivedEvent.data.change).to.eq(relationshipChangedEvent.data.changes[0])
                expect(onboardingChangeReceivedEvent.data.identity).to.exist
