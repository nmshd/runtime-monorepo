...
import { IdentityAttribute, PeerSharedAttributeSucceededNotificationItem } from "@vermascht/content"
import { TransportLoggerFactory } from "@vermascht/transport"
...
export class PeerSharedAttributeSucceededNotificationItemProcessor extends AbstractNotificationItemProcessor<PeerSharedAttributeSucceededNotificationItem> {
    ...
    public override async checkPrerequisitesOfIncomingNotificationItem(
        ...
        if (notificationItem.successorContent instanceof IdentityAttribute) {
            const validationResult =
                await this.consumptionController.attributes.validatePeerSharedIdentityAttributeSuccession(
        ...
        const validationResult =
            await this.consumptionController.attributes.validatePeerSharedRelationshipAttributeSuccession(
                notificationItem.predecessorId,
                successorParams
            )
        return validationResult
    ...
    public override async process(
        notificationItem: PeerSharedAttributeSucceededNotificationItem,
        notification: LocalNotification
    ): Promise<PeerSharedAttributeSucceededEvent> {
        const successorParams = {
            id: notificationItem.successorId,
            content: notificationItem.successorContent,
            shareInfo: { notificationReference: notification.id, peer: notification.peer }
        ...
        try {
            if (notificationItem.successorContent instanceof IdentityAttribute) {
                ;({ predecessor, successor } =
                    await this.consumptionController.attributes.succeedPeerSharedIdentityAttribute(
                        notificationItem.predecessorId,
                        successorParams,
                        false
                    ))
                ...
                ;({ predecessor, successor } =
                    await this.consumptionController.attributes.succeedPeerSharedRelationshipAttribute(
                        notificationItem.predecessorId,
                        successorParams,
                        false
                    ))
    ...
    private async rollbackPartialWork(
        notificationItem: PeerSharedAttributeSucceededNotificationItem,
        ...
        const predecessor = await this.consumptionController.attributes.getLocalAttribute(
            notificationItem.predecessorId
        )
        if (typeof predecessor?.succeededBy !== "undefined") {
            ...
            await this.consumptionController.attributes
                ...
                .catch((e) =>
                    this._logger.error(
                        `Update failed for attribute (attribute id: ${notificationItem.predecessorId}).`,
                        e
                    )
                )
        }
