...
import { Notification, NotificationItem } from "@vermascht/content"
import { CoreId, Message, SynchronizedCollection, CoreErrors as TransportCoreErrors } from "@vermascht/transport"
        ...
        try {
            for (const item of notification.content.items) {
                ...
                if (checkResult.isError()) {
                    ...
                    throw new Error(
                        `Prerequisites of notificationItem with index '${index}' of Notification ${notification.id} not met. Root cause: ${checkResult.error}`
                    )
                }
