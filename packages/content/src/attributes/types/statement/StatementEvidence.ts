...
export class StatementEvidence extends AbstractString {
    ...
    @validate({
        customValidator: (v) =>
            !Object.values(Evidence).includes(v) ? `must be one of: ${Object.values(Evidence)}` : undefined
    })
    public override value: Evidence

    public static override get valueHints(): ValueHints {
        return super.valueHints.copyWith({
            values: Object.values(Evidence).map((value) =>
                ValueHintsValue.from({ key: value, displayName: `i18n://attributes.values.StatementEvidence.${value}` })
            )
        })
    }
