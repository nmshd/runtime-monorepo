...
import {
    ...
    IDigitalIdentityDescriptor
} from "./DigitalIdentityDescriptor"
import {
    IStatementIssuerConditions,
    StatementIssuerConditions,
    StatementIssuerConditionsJSON
} from "./StatementIssuerConditions"
    ...
    public static override postFrom<T extends Serializable>(value: T): T {
        ...
        if (value.predicate.value === Predicates.HasAttribute && (value.object.attributes?.length ?? 0) < 1) {
            throw new ValidationError(
                ...
                `If the predicate of the Statement is '${
                    Predicates.HasAttribute
                }' you have to define attributes in '${nameOf<AbstractStatement>(
                    (x) => x.object
