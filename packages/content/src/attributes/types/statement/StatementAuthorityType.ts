...
export class StatementAuthorityType extends AbstractString {
    ...
    @validate({
        customValidator: (v) =>
            !Object.values(Authority).includes(v) ? `must be one of: ${Object.values(Authority)}` : undefined
    })
    ...
    public static from(
        value: IStatementAuthorityType | Omit<StatementAuthorityTypeJSON, "@type"> | string
