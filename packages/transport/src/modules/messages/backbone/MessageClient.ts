...
export class MessageClient extends RESTClientAuthenticate {
    public async getMessages(
        request?: BackboneGetMessagesRequest
    ): Promise<ClientResult<Paginator<BackboneGetMessagesResponse>>> {
        return await this.getPaged<BackboneGetMessagesResponse>("/api/v1/Messages", request)
    }

    public async createMessage(
        input: BackbonePostMessagesRequest
    ): Promise<ClientResult<BackbonePostMessagesResponse>> {
