...
export class MessageController extends TransportController {
    ...
    private async decryptOwnEnvelope(
        envelope: MessageEnvelope,
        secretKey: CryptoSecretKey
    ): Promise<MessageContentWrapper> {
    ...
    ): Promise<[MessageContentWrapper, CryptoSecretKey]> {
        ...
        if (this.parent.identity.isMe(envelope.createdBy)) {
            if (!secretKey) {
                throw new TransportError(
                    `The own message (${envelope.id.toString()}) could not be decrypted, because no secret key was passed for it.`
                )
            }
