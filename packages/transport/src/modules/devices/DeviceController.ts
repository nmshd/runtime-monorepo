...
export class DeviceController extends TransportController {
    ...
    public async changePassword(newPassword: string): Promise<void> {
        ...
        try {
            ...
            this.log.warn(
                `We've changed the device password on the backbone but weren't able to store it to the database. The new password is '${newPassword}'.`
            )
            throw e
        }
    }
    ...
    public async verify(content: CoreBuffer, signature: CryptoSignature): Promise<boolean> {
        if (!this.publicKey) {
            throw new TransportError(
                "The device has no public key to verify a signature. This can happen if the device is not onboarded yet."
            )
        }
